...
const isEditorReady = computed(() => {
  return editor.value && editor.value.view && editor.value.view.dom;
});
...
watch(
  ...
  ([currentEditor, element]) => {
    if (currentEditor && currentEditor.view && currentEditor.view.dom && element && !element.firstChild) {
      // If editor is ready but not yet mounted to this element
      element.append(currentEditor.view.dom);
      console.log('Editor monté dans le DOM !');
    }
  },
  { immediate: true },
);
...
onMounted(() => {
  console.log('TiptapContent monté, editor:', editor.value);
  if (
    isEditorReady.value &&
    editorElement.value &&
    ...
    console.log('Editor forcé dans onMounted !');
  }
