...
class Job(BaseModel, WorkspaceableMixin):
    ...
    customer_contact: Contact = edgy.ForeignKey(Contact, on_delete="CASCADE", null=True, blank=True)
    priority: str = fields.ChoiceField(JobPriority, default=JobPriority.NORMAL, label="Priorité")
    site: Site = edgy.ForeignKey(Site, on_delete="CASCADE", null=True, blank=True)
